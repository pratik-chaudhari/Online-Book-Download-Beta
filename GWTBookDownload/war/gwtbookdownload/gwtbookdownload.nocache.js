...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "gwtbookdownload" because\n';
  ...
  $wnd.__gwt_activeModules['gwtbookdownload'] = {
    'moduleName' : 'gwtbookdownload',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/gwtbookdownload.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['gwtbookdownload'].superdevmode = true;

  var expectedSuffix = '/gwtbookdownload.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + gwtbookdownload + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:gwtbookdownload';
  var devModeSessionKey = '__gwtDevModeSession:gwtbookdownload';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/gwtbookdownload/gwtbookdownload.nocache.js';
  ...
  $wnd.__gwt_activeModules['gwtbookdownload'].canRedirect = true;
  $wnd.__gwt_activeModules['gwtbookdownload'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING gwtbookdownload';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/gwtbookdownload?user.agent=' + ua + '&_callback=' + callback;
